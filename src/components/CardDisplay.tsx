...
import FoxokisoTotem from './FoxokisoTotem';
...
const CardDisplay: React.FC<CardDisplayProps> = ({ data, index, compact = false, lang }) => {
  ...
  const handleReadMeaning = (e: React.MouseEvent) => {
    ...
    let textToRead = "";
    // PRIESTESS SETTINGS: Slightly higher pitch for 'divine' female tone
    let pitch = 1.2; 
    let rate = 0.9; 

    if (lang === 'zh') {
      if (data.isReversed) {
        textToRead = "祭司逆位解讀... " + data.card.name + "... 勿驚慌，慈悲指引： " + data.card.meaningReversed;
      } else {
        textToRead = "祭司正位解讀... " + data.card.name + "... 神聖指引是： " + data.card.meaningUpright;
      }
    } else {
      if (data.isReversed) {
        textToRead = "Priestess reading Reversed... " + data.card.name + "... Do not fear... The divine guidance is: " + data.card.meaningReversed;
      } else {
        textToRead = "Priestess reading Upright... " + data.card.name + "... The divine meaning is: " + data.card.meaningUpright;
      }
    }
    ...
    if (lang === 'zh') {
      ...
      // Prefer Google Chinese Female if available
      preferredVoice = 
        zhVoices.find(v => v.name.includes('Google') && !v.name.includes('Male')) ||
        zhVoices.find(v => v.name.includes('Yating') || v.name.includes('Huihui') || v.name.includes('Meijia')) ||
        zhVoices[0];
    } else {
      ...
      >
        ...
        <div className="absolute inset-0 w-full h-full backface-hidden">
          <div className="w-full h-full bg-white border-double border-4 border-primary/30 rounded-2xl flex flex-col relative overflow-hidden shadow-inner">
            ...
            {/* V4.9.5 FOXOKISO WATERMARK ON FRONT */}
            <div className="absolute inset-0 flex items-center justify-center opacity-5 pointer-events-none">
               <FoxokisoTotem className="w-48 h-48 text-primary" color="#b91c1c" />
            </div>
                 ...
                 <div className="relative bg-gradient-to-br from-red-600 to-primary text-white px-4 py-2 rounded shadow-inner border border-white/30 ring-1 ring-black/10 flex items-center gap-2">
                    ...
                    <div className="flex flex-col items-start leading-none">
                       <div className="flex items-center gap-1 mb-0.5">
                         ...
                         {/* UPDATED VERSION TO V4.9.9 */}
                         <span className="text-[8px] text-red-100 font-bold tracking-[0.2em] uppercase">V4.9.9</span>
                       </div>
          ...
          <div className="w-full h-full bg-[#fffbf7] border-double border-4 border-primary/20 rounded-2xl flex flex-col p-3 relative overflow-hidden shadow-inner">
            ...
            <div className="mt-2 pt-2 border-t border-primary/10 relative z-10 text-center shrink-0">
              <button 
                ...
                className={`
                  w-full py-2 rounded-xl font-bold text-xs flex items-center justify-center gap-2 transition-all duration-300 shadow-sm hover:shadow-md hover:-translate-y-0.5 relative overflow-hidden
                  ${isReading 
                ...
                `}
              >
                {/* PRIESTESS SOUND WAVE VISUAL */}
                {isReading && (
                  <div className="absolute inset-0 flex items-center justify-center gap-1 opacity-30">
                     <div className="w-1 h-3 bg-white animate-pulse" style={{animationDuration: '0.6s'}}></div>
                     <div className="w-1 h-5 bg-white animate-pulse" style={{animationDuration: '0.4s'}}></div>
                     <div className="w-1 h-2 bg-white animate-pulse" style={{animationDuration: '0.7s'}}></div>
                     <div className="w-1 h-4 bg-white animate-pulse" style={{animationDuration: '0.5s'}}></div>
                  </div>
                )}

                <span className={`text-base rounded-full w-6 h-6 flex items-center justify-center shrink-0 border shadow-sm ${isReading ? 'bg-purple-800 border-purple-700' : 'bg-white border-purple-100'}`}>
                    {isReading ? '🔊' : '🧝‍♀️'}
                </span>
                <span className="text-[10px] md:text-xs tracking-wider text-left leading-tight truncate uppercase font-black z-10">
                  {isReading 
                    ? 'PRIESTESS SPEAKING...' 
            ...
            {/* --- UPDATED FOOTER: 28825252.XYZ (OFFICIAL FORTRESS) --- */}
            <div className="mt-auto pt-2 text-center relative z-10 border-t border-gray-200/50 shrink-0 flex items-center justify-center gap-2 pb-1 bg-white/50 rounded-b-lg">
               {/* MINI GOLDEN TOTEM */}
               <FoxokisoTotem className="w-6 h-6 text-gold drop-shadow-sm" color="#d4af37" />
               <div className="flex flex-col items-start">
                   <a href="https://28825252.xyz" target="_blank" rel="noopener noreferrer" className="text-[9px] font-black tracking-widest text-gray-900 hover:text-primary uppercase no-underline leading-none">
                     28825252.XYZ
                   </a>
                   <span className="text-[6px] text-gray-400 tracking-[0.2em] uppercase">OFFICIAL DIVINE CERTIFICATE</span>
               </div>
            </div>
